...
export const setThemeColor = (
	...
	if (isString(color)) {
		...
		colorList.light = generatePalette(rgbaColor.r, rgbaColor.g, rgbaColor.b, rgbaColor.a, false).map((color) => {
			return rgbaColor2string(color)
		})
		colorList.dark = generatePalette(rgbaColor.r, rgbaColor.g, rgbaColor.b, rgbaColor.a, true).map((color) => {
			return rgbaColor2string(color)
		})
	} else {
