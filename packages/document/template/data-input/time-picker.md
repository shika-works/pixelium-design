...
<preview path="./time-picker-basic.vue"></preview>
...
<preview path="./time-picker-range.vue"></preview>
...
<preview path="./time-picker-quick.vue"></preview>
...
<preview path="./time-picker-splitter.vue"></preview>
...
<preview path="./time-picker-format.vue"></preview>
...
<preview path="./time-picker-more.vue"></preview>
...
[[[api zh
modelValue: 时间选择器的值（受控模式），支持 `v-model`。
defaultValue: 时间选择器的默认值（非受控模式）。
mode: 时间选择的模式。
placeholder: 占位符文本。
...
events.input: 时间选择器输入时的回调。
events.update:modelValue: 更新 `modelValue` 的回调。
events.change: 时间选择器值变化时的回调。
...
timePickerExpose.focus: 聚焦当前控件，范围选择时不传 `placement` 默认作用于开始时间。
timePickerExpose.blur: 取消聚焦当前控件，范围选择时不传 `placement` 默认作用于开始时间。
timePickerExpose.clear: 清空当前输入内容。
timePickerExpose.select: 选中当前输入内容，范围选择时不传 `placement` 默认作用于开始时间。
]]]

[[[api en
modelValue: The value of the time picker (controlled mode), supports `v-model`.
defaultValue: The default value of the time picker (uncontrolled mode).
...
size: Size of the time picker input box.
shape: Shape of the time picker input box.
...
events.input: Callback when the time picker input is triggered.
events.update:modelValue: Callback for updating `modelValue`.
events.change: Callback when the time picker value changes.
...
timePickerExpose.focus: Focuses the current control; for range selection, defaults to the start time if `placement` is not provided.
timePickerExpose.blur: Blurs the current control; for range selection, defaults to the start time if `placement` is not provided.
timePickerExpose.clear: Clears the current input content.
timePickerExpose.select: Selects the current input content; for range selection, defaults to the start time if `placement` is not provided.
]]]
