...
单选框 Radio 有两种样式变体，普通模式（`normal`，默认）和经典模式（`retro`）。经典模式的样式专门为致敬上世纪的游戏机 UI 而设置
...
The Radio component has two style variants: normal mode (`normal`, default) and retro mode (`retro`). The retro mode style is specifically designed as a tribute to the game console UI of the last century.
